...
class Services(object):
    def __init__(self):
        ...
        self.main_url = r'https://services1.arcgis.com/IOnDXYLCAWAfoO54/ArcGIS/rest/services/service_17c106ac35964f64be6708ff48abd974/FeatureServer/'

        self.GPT_DGAR_FASUB = "0"
        # self.TB_FA_ANLQUIMICO = "1"
        # self.TB_FA_ANLRADIANC = "2"
        self.TB_FA_MULTIMEDIA = "3"
    ...
    @property
    def query_url(self):
        return ''.join([self.main_url, self.GPT_DGAR_FASUB, '/query?token={}'.format(self.token)])

    # Url para la consulta de imagenes (Photos)
    @property
    def query_url_attach_Photos(self):
        return ''.join([self.main_url, self.TB_FA_MULTIMEDIA, '/queryAttachments?token={}'.format(self.token)])

    # Url para la eliminacion de registros de ficha POG
    @property
    def delete_url_GPT_FA_SUB(self):
        return ''.join([self.main_url, self.GPT_DGAR_FASUB, '/deleteFeatures?token={}'.format(self.token)])
